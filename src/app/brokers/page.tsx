...
import { mockBrokers } from "@/constants/realestate";
import { ChevronLeft, Gem, Phone } from "lucide-react";
...
function BrokersContent() {
  ...
  return (
    <div>
      <div className="top-0 border-y border-gray-200 bg-white py-4 lg:sticky">
        <div className="container mx-auto px-4 lg:max-w-[1300px]">
          ...
          <div>
            <div
              className={`grid grid-cols-1 gap-3 md:min-w-[400px] lg:grid-cols-2 xl:grid-cols-8`}
            >
              {/* 1. Tab Switcher */}
              ...
              <div>
                <button
                  ...
                  className="flex w-full items-center justify-center rounded-lg bg-main px-4 py-3 text-sm font-medium text-white hover:bg-main-dark"
                >
                  ابحث
                </button>
              </div>
        ...
        <div className="container mx-auto px-4 lg:max-w-[1300px]">
          <div className="grid gap-4 md:grid-cols-9">
            <div className="md:col-span-5 lg:col-span-7">
              <div className="min-h-[400px] w-full rounded-xl bg-gradient-to-b from-main-transparent to-white p-6">
                <span className="block w-fit rounded-lg bg-main px-3 py-1 font-bold text-white">
                  TruBroker™
                </span>
                <p className="mt-2 text-sm">
                  تعرف على الوكلاء الذين يتمتعون بسجل حافل من معدلات الاستجابة
                  العالية والإعلانات الحقيقية.
                </p>
                  ...
                  {mockBrokers.map((broker) => {
                    return (
                      <Link
                        ...
                        className="flex gap-4 rounded-lg border border-gray-100 bg-white p-2 shadow-sm"
                        key={broker.id}
                      >
                        <Image
                        ...
                        />
                        <div>
                          <h2 className="mb-3 font-semibold">{broker.name}</h2>
                          <div className="mb-1 line-clamp-1 flex items-center gap-1 text-xs">
                            نطاق عمله
                            <p className="max-w-[150px] truncate font-bold">
                              {broker.jurisdictioAreas.join(" , ")}
                            </p>
                          </div>
                          <div className="mb-3 line-clamp-1 flex items-center gap-1 text-xs">
                            يتحدث{" "}
                            <p className="max-w-[150px] truncate font-bold">
                              {broker.language.join(" , ")}
                            </p>
                          </div>
                          <div className="flex gap-2">
                            <span className="rounded-md border border-main bg-white px-2 py-1 text-xs text-main">
                              {broker.sale} بيع
                            </span>
                            <span className="rounded-md border border-main bg-white px-2 py-1 text-xs text-main">
                              {broker.rent} ايجار
              ...
              <Link
                className="mx-auto flex w-fit items-center gap-2 rounded-md bg-main-transparent px-3 py-2 text-main transition hover:bg-main hover:text-white"
                href={"/brokers/search"}
              >
                عرض كل ™TruBrokers <ChevronLeft size={16} />
              </Link>
            </div>
            <div className="md:col-span-4 lg:col-span-2">
              ...
              <div className="flex flex-col gap-3 py-4">
                ...
                <div className="rounded-lg bg-main-transparent p-4">
                  ...
                  <p className="mt-3 text-center text-sm text-main">
                    أثبت الوكلاء الذين يحملون شارة معلن بجودة عالية أنهم يقدمون
                    إعلانات متاحة وعالية الجودة.
                  </p>
                </div>
                <div className="rounded-lg bg-main-transparent p-4">
                  <span className="mx-auto flex w-fit items-center gap-2 rounded-lg bg-main px-3 py-1 text-sm font-bold text-white">
