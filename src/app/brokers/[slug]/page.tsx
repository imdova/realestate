...
import { mockBrokers, realEstateData } from "@/constants/realestate";
import { useAppSettings } from "@/hooks/useAppSettings";
import { Gem, ListFilter, MessageSquare, Phone } from "lucide-react";
...
const tabs = [{ label: "الملف" }, { label: "الاعلانات الفعاله" }];
...
function renderBadges(badges: string[]) {
  return badges.map((badge, index) => {
    ...
    return (
      <span
        key={index}
        className="flex gap-2 rounded-md bg-main px-2 py-1 text-[10px] font-medium text-white md:px-3 md:py-2 md:text-xs"
      >
        {badgeContent}
      </span>
    );
...
const BrokerProfileContent = ({ params }: BrokerProfilePageProps) => {
  ...
  const [tab, setTab] = useState("الملف");
  ...
  const broker = mockBrokers.find((broker) => broker.id === slug);
  if (!broker) {
    return notFound();
  }
  const isTruBroker = broker.badges.find((badge) => badge === "TruBroker");
      ...
      <div>
        <section className="border-b border-gray-200">
          <div className="container mx-auto px-4 lg:max-w-[1300px]">
            <div className="relative min-h-[280px] py-2 md:py-8">
              {isTruBroker ? (
                <div className="absolute flex h-[100px] w-full items-center justify-end rounded-2xl bg-[url(/images/shap.png)] bg-cover bg-center p-3 text-xl font-bold text-white md:text-3xl">
                  TruBroker™
                </div>
              ) : (
                <div className="absolute h-[100px] w-full rounded-2xl bg-gradient-to-r from-main-transparent from-10% via-white via-30% to-main-transparent to-90%"></div>
              )}
              <div className="relative px-4 py-6 md:px-8">
                <div className="flex gap-4 lg:flex-row lg:items-end">
                  <div className="flex flex-1 flex-col gap-4 pt-6 md:flex-row md:items-end lg:pt-0">
                    <Image
                      className="h-[90px] w-[90px] rounded-full object-cover md:h-[200px] md:w-[200px]"
                      src={broker.image ?? "/images/user-placholder.jpg"}
                      width={400}
                      height={400}
                      objectFit="cover"
                      alt={broker.name}
                    ...
                    <div className="flex flex-1 flex-col justify-between gap-3 lg:flex-row lg:items-end">
                      <div>
            ...
            <div className="grid grid-cols-9 gap-6">
              <div className="col-span-6">
                ...
                <div className="my-6 flex justify-between gap-2">
                  <span>
                    عرض {currentPage} - {totalPages} من {totalItems} عقار مرتبة
                    حسب
                  </span>
              ...
              <div className="col-span-3">
                <div className="flex flex-col gap-3 p-3">
                  ...
                  {broker.language && (
                    <p className="text-sm">
                      <b>اللغات: </b> {broker.language.join(" , ")}
                    </p>
                  )}
                  {broker.practices && (
                    <p className="text-sm">
                      <b>الممارسات: </b> {broker.practices?.join(" , ")}
                    </p>
                  )}
                  {broker.jurisdictioAreas && (
                    <p className="text-sm">
                      <b>مناطق الاختصاص: </b>{" "}
                      {broker.jurisdictioAreas.join(" , ")}
                    </p>
                  )}
                  {(broker.sale || broker.rent) && (
                    <p className="text-sm">
                      <b>عقارات: </b> {broker.sale && <>للبيع: {broker.sale}</>}{" "}
                      {"  "}{" "}
                      {broker.rent && (
                        <>
                          للايجار:
...
const BrokerProfilePage = ({ params }: BrokerProfilePageProps) => {
  return (
    <Suspense>
      <BrokerProfileContent params={params} />
    </Suspense>
  );
};
