...
function BlogContent({ searchParams }: PageProps) {
...
export default function BlogHomePage({ searchParams }: PageProps) {
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 lg:max-w-[1200px]">
        <div className="relative flex h-[400px] w-full items-center justify-center rounded-lg bg-[url(/images/building.jpg)] bg-cover p-4 md:p-6">
          <div className="absolute inset-0 flex flex-col justify-end gap-2 bg-gradient-to-t from-black/60 to-transparent p-6">
            <h1 className="text-xl font-extrabold leading-tight text-white sm:text-3xl">
              تعرف على موقع عقار مصر ومدونته باللغة العربية
            </h1>
            <div className="flex items-center gap-2">
              <Link
                className="w-fit rounded-md bg-main px-2 py-1 text-xs text-white"
                href="/blog/categories/فعاليات-واخبار-بيوت-مصر"
              >
                فعاليات واخبار بيوت مصر
              </Link>
              <span className="text-xs text-white">2 دقائق للقراءة</span>
            </div>
          </div>
        </div>
